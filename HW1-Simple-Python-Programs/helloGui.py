...
def pickPlayer(event = None):
	...
	if not w.get():
		...
		if num < 2 : 							#loop below will run forever
			label = Label(master, text= "Enter more than 2!")	
			label.grid(column = 0, row = 2)
			return
		for a in range(num):
		...
		if count == 1:							
			label = Label(master, text=	randWord)
...
box = Label(master,text = "Enter a list of at least 2 names\n separated only by a comma\n i.e. 'Joe,Bob,Chris'\n")
